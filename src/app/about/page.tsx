      ...
      <section style={{ padding: '2rem 2rem 5rem 2rem' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '4rem' }}>
          ...
          <div className="grid-cols-2" style={{ alignItems: 'center' }}>
            <div>
              <h2 style={{ fontSize: '1.8rem', marginBottom: '1rem', color: '#0f172a' }}>{lang === 'zh' ? '公会资质监管条例' : lang === 'bm' ? 'Mandat Kawal Selia Kami' : 'Our Regulatory Mandate'}</h2>
            ...
            <div className="card" style={{ 
              ...
              padding: '2.5rem', 
              background: '#ffffff', 
              borderColor: 'rgba(10, 186, 181, 0.15)',
              boxShadow: '0 10px 30px rgba(0,0,0,0.04)',
              borderLeft: '4px solid var(--primary)',
              borderRadius: '16px'
            }}>
              <h3 style={{ color: 'var(--primary)', fontSize: '1.3rem', fontWeight: 700, marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '0.6rem', fontFamily: 'Outfit' }}>
                🛡️ {lang === 'zh' ? '三大核心安全支柱' : lang === 'bm' ? 'Tiga Pilar Kualiti Teras' : 'Core Quality Pillars'}
              </h3>
              <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem', fontSize: '0.92rem', listStyle: 'none', padding: 0, margin: 0 }}>
                <li style={{ display: 'flex', gap: '0.6rem', color: '#334155', alignItems: 'center' }}><CheckCircle size={18} style={{ color: '#10b981', flexShrink:0 }} /> {lang === 'zh' ? '结核病胸部 X 光合格证' : lang === 'bm' ? 'Kelulusan Pemeriksaan X-Ray TB Mandatori' : 'Mandatory Chest X-Ray TB Clearance'}</li>
                <li style={{ display: 'flex', gap: '0.6rem', color: '#334155', alignItems: 'center' }}><CheckCircle size={18} style={{ color: '#10b981', flexShrink:0 }} /> {lang === 'zh' ? '认证学术水平与急救证书' : lang === 'bm' ? 'Sijil Kecekapan & Pertolongan Cemas Disahkan' : 'Verified Academic & First-Aid Certifications'}</li>
                <li style={{ display: 'flex', gap: '0.6rem', color: '#334155', alignItems: 'center' }}><CheckCircle size={18} style={{ color: '#10b981', flexShrink:0 }} /> {lang === 'zh' ? '统一临床标准工作守则' : lang === 'bm' ? 'Prosedur Operasi Standard (SOP) Bersepadu' : 'Unified Standard Operating Procedures (SOPs)'}</li>
                <li style={{ display: 'flex', gap: '0.6rem', color: '#334155', alignItems: 'center' }}><CheckCircle size={18} style={{ color: '#10b981', flexShrink:0 }} /> {lang === 'zh' ? '持续专业职业培训积分' : lang === 'bm' ? 'Mata Pembangunan Profesional Berterusan' : 'Continuous Professional Development Points'}</li>
              </ul>
            </div>
          </div>
          ...
          <div className="card" style={{ 
            ...
            <h3 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#0f172a', marginBottom: '1.25rem', textAlign: 'center', fontFamily: 'Outfit' }}>
              🏥 {lang === 'zh' ? '医院与卫生机构合作前景' : lang === 'bm' ? 'SOP Rangkaian Hospital & Kerjasama' : 'Hospital SOP Frameworks & Collaboration'}
            </h3>
            <p style={{ color: '#475569', lineHeight: 1.6, textAlign: 'center', maxWidth: '800px', margin: '0 auto 1.75rem auto', fontSize: '0.95rem' }}>
            ...
            <div style={{
              ...
              <p style={{
                ...
                🤝 <strong style={{ color: 'var(--primary)' }}>{lang === 'zh' ? '欢迎建立官方合作伙伴关系：' : lang === 'bm' ? 'Peluang Kerjasama Rakan Hospital & Klinik:' : 'Healthcare Partner Collaboration Invitation:'}</strong>
                <span>
                  {lang === 'zh'
      ...
      <section style={{ padding: '4rem 2rem', borderTop: '1px solid rgba(255,255,255,0.03)', backgroundColor: 'rgba(15, 23, 42, 0.2)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
            ...
            <h2 style={{ fontSize: '2.2rem', marginBottom: '0.5rem', color: '#0f172a' }}>{lang === 'zh' ? '专属资质审查基地与实操模拟演练' : lang === 'bm' ? 'Pusat Latihan Penilaian & Latihan Simulasi Penjaga' : 'Designated Vetting Base & Vitals Vetting Simulations'}</h2>
            ...
            {photos.map((photo, index) => (
              ...
              >
                ...
                <div style={{ padding: '1.5rem' }}>
                  ...
                  <h4 style={{ color: '#0f172a', fontSize: '1.15rem', marginBottom: '0.5rem', fontWeight: 700 }}>{photo?.caption || ''}</h4>
